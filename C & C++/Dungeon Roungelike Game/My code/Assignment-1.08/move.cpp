...
void do_combat(dungeon *d, character *atk, character *def)
{
  ...
  if (def->alive) {
    ...
    if (def != d->PC) {
      //Lee's
      if((static_cast<npc*>(def)->abilities & NPC_UNIQ) == NPC_UNIQ){
        for(int i =0; (unsigned)i < d->monster_descriptions.size();i++){
          if(static_cast<npc*>(def)->name.compare(d->monster_descriptions.at(i).get_name())==0){
            d->killed_unique_monster_list.push_back(i);
            break;
          }
        }
      }
