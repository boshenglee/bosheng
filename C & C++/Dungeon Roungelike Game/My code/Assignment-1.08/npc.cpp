...
void gen_monsters(dungeon *d)
{
  ...
  for (i = 0; i < d->num_monsters; i++) {
    ...
    do{
      valid = 1;
      unique = 0;
      mon_random = rand() % d->monster_descriptions.size();
      m = d->monster_descriptions.at(mon_random).generate_npc();

      if((m->abilities & NPC_UNIQ) == NPC_UNIQ){
        //check whether exist or not
        for(int i =0; (unsigned)i<d->unique_monster_list.size();i++){ 
          if(d->unique_monster_list.at(i)==mon_random){
            valid  = 0;
          }
        }
        //check whether unique monster is killed
        for(int i =0; (unsigned)i<d->killed_unique_monster_list.size();i++){ 
          if(d->killed_unique_monster_list.at(i)==mon_random){
            valid  = 0;
          }
        }
        unique = 1;
      }
      uint32_t random_rarity = rand() % 100;
      if(random_rarity >= m->rarity){
        valid = 0;
      }
    ...
    if(unique ==1){
      d->unique_monster_list.push_back(mon_random);
    }


    // for(int i =0; (unsigned)i<d->unique_monster_list.size();i++){
    //    mvprintw(i,0, "%d",d->unique_monster_list.at(i));
    // }
    // refresh();
    // getch();
    do {
