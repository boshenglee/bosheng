...
{
  //generate 10 objects and place it in the map.
  for (int i = 0; i < 10; i++)
  {
    real_object *new_obj;
    while (true)
    {
      int random_obj = rand() % d->object_descriptions.size();
      new_obj = d->object_descriptions[random_obj].generate_obj();
      int random_num = rand() % 100;
      if (random_num < (int) new_obj->rarity)
      {
        break;
      }
    }
    //find a location place it.
    uint32_t room;
    pair_t p;
    do
    {
    ...
    } while (d->object_map[p[dim_y]][p[dim_x]] != 0);

    new_obj->position[dim_x] = p[dim_x];
    new_obj->position[dim_y] = p[dim_y];
    d->object_map[p[dim_y]][p[dim_x]] = new_obj;
  }
}