...
void do_moves(dungeon_t *d)
{
  ...
  if (pc_is_alive(d) && e->c == &d->pc)
  {
    ...
    event_delete(e);
    pc_next_pos(d, next);
    if ((next[dim_y] + c->position[dim_y] < DUNGEON_Y - 1 && next[dim_y] + c->position[dim_y] > 0) && (next[dim_x] + c->position[dim_x] < DUNGEON_X - 1 && next[dim_x] + c->position[dim_x] > 0))
    {
      next[dim_x] += c->position[dim_x];
      next[dim_y] += c->position[dim_y];
      if (mappair(next) > ter_wall_immutable)
      {
        move_character(d, c, next);
        dijkstra(d);
        dijkstra_tunnel(d);
