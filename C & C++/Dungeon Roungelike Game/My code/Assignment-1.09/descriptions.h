...
class object_description {
 ...
 public:
  object_description() : name(),    description(), type(objtype_no_type),
                         ...
                         num_found(0), num_expunged(0) //Lee's
  {
  }
  inline bool can_be_generated()
  {
    return !num_expunged && (!artifact || (artifact && !num_generated && !num_found)); //lee's
  }
  inline bool pass_rarity_roll()
  {
    return rarity > (unsigned) (rand() % 100);
  ...
  inline void expunged() {num_expunged = 1;} //Lee's
};
